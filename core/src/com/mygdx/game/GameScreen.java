...
public class GameScreen implements Screen {
    ...
    private boolean addedNewIncoming = false;
    ...
    public GameScreen(Game aGame) {
        try{
            ...
            road1 = new Image(new Texture(Constants.level1BackgroundPath));
            road1.setPosition(0,0);
            road2 = new Image(new Texture(Constants.level1BackgroundPath));
            ...
            bodyDef.position.set(Constants.screenWidth / Constants.metersToPixels, 0);
            // Create a body in the world using our definition
            ...
            shape.setAsBox(1f, 1000f);

            FixtureDef fixtureDef = new FixtureDef();
            fixtureDef.shape = shape;
            fixtureDef.density = 1f;
            Fixture left_fixture = leftBarrier.createFixture(fixtureDef);
            Fixture right_fixture = rightBarrier.createFixture(fixtureDef);
            shape.dispose();
    ...
    private void update(float delta) {
        ...
        if(carPosY % 1000 > Constants.spawnLimit){
            if(!addedNewIncoming){
                entities.add(new Incoming(world, stage, carPosY + Constants.spawnLimit));
                addedNewIncoming = true;
            }
        }
        else{
            addedNewIncoming = false;
        }

        if(carPosY > Constants.nextLevel){
